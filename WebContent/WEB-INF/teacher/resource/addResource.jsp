...
<script type="text/javascript">
	function addOneFile(obj) {
		...
		//2 判断是否已经选择文件
		var file = document.getElementById("file"+(table.rows.length-4)).value;
		if(file == null || file == "") {
		 	alert("请先选择文件");
		 	return;
		}
		...
		tr.innerHTML = "<td></td><td><input type='file' name='fileURL' id='file"+(table.rows.length-3)+"' style='width: 215px' accept='aplication/zip'/></td><td><input type='button' value='删除文件' onclick='delOneFile(this)'  /></td>";
		//4 找到表格最后一行
...
<body
	...
	<form
		action="${pageContext.request.contextPath}/WebMaterialServlet?action=addResource"
